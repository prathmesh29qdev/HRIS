...
function highlightMonthYear() {
	...
	else { this.className = 'monthYearActive'; activeSelectBoxMonth = this; }
...
function selectMonth() {
	...
	document.getElementById('monthDropDown').style.display = 'none';
	EIS_Hide_Frame();
	for (var no = 0; no < monthArray.length; no++) { document.getElementById('monthDiv_' + no).style.color = ''; }
...
function slideCalendarSelectBox() {
	if (selectBoxMovementInProgress) {
		if (activeSelectBox.parentNode.id == 'hourDropDown') { changeSelectBoxHour(false, activeSelectBox); }
...
function highlightClose() {
	if (this.src.indexOf('over') >= 0) { this.src = pathToImages + 'close.gif'; }
...
function closeCalendar() {
	...
	if (iframeObj) { iframeObj.style.display = 'none'; EIS_Hide_Frame(); }
	if (activeSelectBoxMonth) activeSelectBoxMonth.className = '';
...
function pickDate(e, inputDay) {
	...
	if (returnFormat) {
		...
		for (var no = 0; no < returnDateToYear.options.length; no++) { if (returnDateToYear.options[no].value == currentYear) { returnDateToYear.selectedIndex = no; break; } }
		...
		if (calendarDisplayTime) {
			for (var no = 0; no < returnDateToHour.options.length; no++) { if (returnDateToHour.options[no].value == currentHour) { returnDateToHour.selectedIndex = no; break; } }
	...
	if (inputField.value.length > 0) {
		if (!format.match(/^[0-9]*?$/gi)) {
			...
			for (var no = 0; no < positionArrayNumeric.length; no++) {
				...
				itemIndex++;
				if (positionArrayNumeric[no] == positionArray['m']) { currentMonth = items[itemIndex] - 1; continue; }
...
function displayCalendarSelectBox(yearInput, monthInput, dayInput, hourInput, minuteInput, buttonObj) {
	...
	if (hourInput) { currentHour = hourInput.options[hourInput.selectedIndex].value; inputHour = currentHour / 1; }
	...
	if (!calendarDiv) { initCalendar(); } else { writeCalendarContent(); }
