...
String cqtydisp = "",  para1 = "", para2 = "", para3="", v2 = "", str="",recrdno="0",recrdno_engg="0";
long rw1 = 0, i=1, balance=0, updated=0;
...
status = stockBean.insertStock(con, branchid, partid, pid, date, 0, Long.parseLong(cqty), 0, usrid, 0, 0, 0, 0, 0, "DEBIT", 2, 0);
/*
...
if(status == 1)
{
	...
	recrdno_engg=rec.getRecordid(date, esid);

	str="select balance from ENG_STOCK_DETAILS where p_id ="+partid+" and pmodel_id="+pid+" and eng_name="+worker+" and RECORD_ID in(select max(RECORD_ID) from ENG_STOCK_DETAILS where p_id ="+partid+" and pmodel_id="+pid+" and eng_name="+worker+" and RECORD_ID<="+Long.parseLong(recrdno_engg)+")";
	rs=stat.executeQuery(str);
	if(rs.next()){v1=""; if((v1=rs.getString(1))!=null){esbal=Double.parseDouble(v1);}}

	esbal=esbal+Double.parseDouble(cqty);
	...
	pst1.setLong(4, pid);
	pst1.setLong(5, Long.parseLong(recrdno_engg));
	pst1.executeUpdate();

	pst2=con.prepareStatement("insert into ENG_STOCK_DETAILS(id, P_ID, ENG_NAME, INWARD, BALANCE, USED_DATE, RECORD_ID, br_id, STOCK_TIME, USER_ID, SRC, pmodel_id) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
	pst2.setLong(1, esid);
	...
	pst2.setLong(7, Long.parseLong(recrdno_engg));
...
while(i<=hcart)
{
	para1=""; para2=""; para3=""; partid=0;cqty="";maxbal=0.0;recrdno="0"; pid=0; status=0;
	...
	maxbal=0.0; recrdno="0"; recrdno_engg="0"; esbal=0.0;
	...
	*/				
	...
	if(status==1)
	{
		...
		str="select balance from ENG_STOCK_DETAILS where p_id ="+partid+" and pmodel_id="+pid+" and eng_name="+worker+" and RECORD_ID in(select max(RECORD_ID) from ENG_STOCK_DETAILS where p_id ="+partid+" and pmodel_id="+pid+" and eng_name="+worker+" and RECORD_ID<='"+recrdno_engg+"')";
		rs=stat.executeQuery(str);
		if(rs.next()){v1=""; if((v1=rs.getString(1))!=null){esbal=Double.parseDouble(v1);}}
		
		esbal=esbal+Double.parseDouble(cqty);
		
		pst1.setLong(1, Long.parseLong(cqty));
		pst1.setLong(2, partid);
		pst1.setLong(3, worker);
		pst1.setLong(4, pid);
		pst1.setLong(5, Long.parseLong(recrdno_engg));
...
if(updatestatus == 0)
{
	...
	%>
	<br><br><br><br><br>
	<div id="done">Failed Allocation!</div>
	<br><br><br><center>
...
if(updatestatus == 1)
{
	...
	%>
	<br><br><br><br><br>
	<div id="done">Allocation Done!</div><br><br><br><center>
...
if(updatestatus == 2)
{
	...
	%>
	<br><br><br><br><br>
	<div id="done">Insufficient part stock for allocation!</div>
	<br><br><br><center>
	<input type=button value="  Back  " onclick="window.history.go(-1)" style="width:200px"></center>
	<%
}
