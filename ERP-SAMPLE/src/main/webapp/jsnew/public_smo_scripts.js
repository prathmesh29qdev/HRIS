...
function checkUncheckAll(theElement)
{
	...
	for(z=0; z<theForm.length;z++){if(theForm[z].type == 'checkbox' && theForm[z].name != 'checkall'){theForm[z].checked = theElement.checked;}	}
}
function checkUncheckGroup(theElement, groupid)
{
	var theForm=theElement.form, z=0;
	for(z=0; z<theForm.length;z++){if(theForm[z].type=='checkbox' && theForm[z].id==groupid){theForm[z].checked = theElement.checked;}}
}
	...
	for(var z=0; z<formElements.length;z++)
	{
		...
		if(theItem)
		{
			...
			else
			{
				...
				{
					if(theItem.childNodes[x]){	if (theItem.childNodes[x].id){nextArray += theItem.childNodes[x].id+',';}	}
				}
